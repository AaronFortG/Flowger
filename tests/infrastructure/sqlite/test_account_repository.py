...
def test_sqlite_account_repository_saves_accounts(tmp_path: Path) -> None:
    ...
    mock_account_1 = Account(id="acc_1", iban="ES11", name="Checking", currency="EUR", bank_name="Imagin", country="ES")
    mock_account_2 = Account(id="acc_2", iban="ES22", name="Savings", currency="EUR", bank_name="Imagin", country="ES")
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, iban, name, currency, bank_name, country FROM accounts ORDER BY id ASC")
    ...
    mock_account_1_updated = Account(
        id="acc_1", iban="ES11", name="Checking Updated", currency="EUR", bank_name="Imagin", country="ES"
    )
...
def test_sqlite_account_repository_returns_domain_objects(tmp_path: Path) -> None:
    ...
    init_db(db_path)
    repo = SqliteAccountRepository(db_path)
    repo.save_accounts([Account(id="acc_x", iban="ES99", name="Test", currency="USD", bank_name="Imagin", country="ES")])
